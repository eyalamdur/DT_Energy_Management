...
def evaluate_models(stats_file, date, env: gym.Env, dt_model, rl_model_names, rl_models, num_episodes: int = 10,
    ...
    with open(f"code/evaluate/stats/{date}/DT.txt", "a") as model_file:
    ...
    for rl_model in rl_model_names:
        print(rl_model)
        with open(f"code/evaluate/stats/{date}/{rl_model}.txt", "a") as model_file:
...
def main():
    ...
    with open(f"code/evaluate/stats/{date}/evaluate_stats.txt", "a") as evaluate_file:
        ...
        evaluate_models(evaluate_file, date, env, dt_model, rl_model_names, rl_models, num_episodes=10, max_episode_length=480)
